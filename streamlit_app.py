...
Finds files in ./data by pattern AND verifies the right sheet, so naming/dates don't break it.
"""
import os, glob
import pandas as pd
...
with tabs[0]:
    st.subheader("Today's buying brief"); st.text(brief)
with tabs[1]:
    if db is None: st.info("Mock database not found.")
    else:
        ...
        st.write(f"{len(v):,} items"); st.dataframe(v, use_container_width=True, height=460)
with tabs[2]:
    if restock is None: st.info("No restock data found.")
    else:
...
with tabs[3]:
    if pricing is None: st.info("No pricing data found.")
    else: st.dataframe(pricing, use_container_width=True, height=460)
